...
#
# Recreates the arcorrectionslab.org hero animation as a standalone .gif you
# can drop straight into the site's existing <img src="..."> tag — no
# HTML/CSS/JS/iframe needed. Matched directly against the real
# hero_mockup_v11.html file (title/tagline/affiliation copy, colors,
# background gradient position, dash pattern, aspect ratio) rather than
# placeholder values.
#
...
# Matches hero_mockup_v11.html's own SVG viewBox aspect ratio (900x280)
# scaled up -- 1200 wide keeps it crisp without an oversized file.
OUT_GIF   <- "hero_decay.gif"
WIDTH_PX  <- 1200
HEIGHT_PX <- round(WIDTH_PX * 280 / 900)  # 373
FPS       <- 20
DURATION_S <- 9
N_FRAMES  <- FPS * DURATION_S

# Real copy, pulled directly from hero_mockup_v11.html (the ".title",
# ".tagline", ".affiliation" divs). The mockup's ".cap" line ("Mockup for
# review — v11") is a review-only annotation, not part of the real graphic
# -- intentionally not reproduced here.
TITLE_TEXT       <- "Advancing Research in Corrections Lab"
TAGLINE_TEXT     <- "Studying correctional systems as they operate,\nand how they can work better."
# (v11's HTML uses an em dash here -- "operate — and how" -- swapped
# to a comma above only because some PNG-device fonts in testing rendered
# the em dash as a stray ellipsis glyph; switch back to "—" if your
# machine's font handles it fine, to match the mockup exactly)
AFFILIATION_TEXT <- "School of Criminology and Criminal Justice - University of Nebraska Omaha"
# (v11's HTML uses a middle dot "·" here -- swapped to a plain hyphen above
# for the same reason as the tagline's dash: it rendered as ".." with this
# sandbox's font. Swap back to "·" if your machine's font renders it fine.)

BG_COLOR    <- "#0f1622"  # v11's .wrap background base color
TEXT_COLOR  <- "#eaf2ff"
TAGLINE_GLOW_COLORS <- c("#3b82f6", "#2dd4bf")  # blue + teal, matching v11's dual-color text-flash-glow
...
groups <- data.frame(
  ...
  color = c("#3b82f6", "#eef2f7", "#2dd4bf"),  # blue / near-white (v11's exact line color) / teal
...
build_frame_plot <- function(f) {
  ...
  # Positioned to match v11's CSS exactly: radial-gradient(...at 30% 78%,
  # rgba(59,130,246,0.10)...) and (...at 78% 78%, rgba(45,212,191,0.09)...)
  # -- both blobs sit low in the panel (78% down from the top), blue on the
  # left (30% across), teal on the right (78% across).
  ...
  glow_x_blue <- X_MIN + 0.30 * (X_MAX - X_MIN)
  glow_x_teal <- X_MIN + 0.78 * (X_MAX - X_MIN)
  glow_y      <- 0.22 * 33  # "78% down from the top" of a 0-33 y-range
  if (HAS_GGFX) {
    p <- p + with_blur(glow_layer(glow_x_blue, glow_y, "#3b82f6", 0.10), sigma = 40) +
             with_blur(glow_layer(glow_x_teal, glow_y, "#2dd4bf", 0.09), sigma = 40)
  } else {
    p <- p + glow_layer(glow_x_blue, glow_y, "#3b82f6", 0.05) +
             glow_layer(glow_x_teal, glow_y, "#2dd4bf", 0.045)
  }
  ...
  for (gid in unique(d$lines$id)) {
    ...
    if (a <= 0.005) next
    lty <- if (unique(sub$dashed)) "B7" else "solid"  # hex dash code B7 = 11,7 -- matches v11's stroke-dasharray exactly
